...
fn get_mod_dir(mc_dir: PathBuf, iris: bool) -> Result<PathBuf, ()> {
    let mod_dir = if iris {
        mc_dir.join("iris-reservec")
    } else {
        mc_dir.join("mods")
    };
    if mod_dir.exists() {
...
#[tauri::command]
async fn download_mods(iris: bool, state: State<'_, AppState>, version: Version) -> Result<(), ()> {
    let mc_dir = get_default_client_dir();
    let mod_dir = get_mod_dir(mc_dir, iris)?;
    let req = state
        .client
